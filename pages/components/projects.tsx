...
export default function MakeProject({ type, title, description1, description2, gitLink, webLink}: { type: string, title: string, description1: string, description2: string, gitLink: string, webLink: string}) {
    ...
    <motion.article className='flex flex-col gap-4' variants={ container } initial='hidden' whileInView='show' viewport={{ once: true, margin: '-100px 0px' }}>
        <motion.header className='flex flex-col font-montreal w-full gap-1'>
            ...
            <motion.h3 className='font-woodland text-7xl mb-2 flex items-center gap-4' variants={ children } >{title ? title : "Default title"}
            </motion.h3>
        ...
        <motion.div className='flex gap-6 text-2xl font-montreal' variants={ children } >
            <motion.a href={gitLink} target='_blank' className='flex items-center gap-2 hover:text-black transition'><Image src='/github.svg' width='24' height='24' alt='GitHub logo' />GitHub</motion.a>
            <motion.a href={webLink} target='_blank' className='flex items-center gap-2 hover:text-black transition'><Image src='/website.svg' width='24' height='24' alt='Icon of a simplistic globe' />Website</motion.a>
        </motion.div>
    </motion.article>
    )
}